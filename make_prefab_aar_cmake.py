...
def build_arch(ndk_abi, dest_abi_code, install_folder: Path):
    ...
    -DCMAKE_BUILD_TYPE=Release -DANDROID_NDK=$ANDROID_NDK_HOME -DANDROID_ABI="{dest_abi_code.split('.')[1]}" -Dprotobuf_BUILD_TESTS=OFF\
     -Dprotobuf_BUILD_SHARED_LIBS=ON -Dprotobuf_BUILD_PROTOC_BINARIES=OFF -Dprotobuf_FORCE_FETCH_DEPENDENCIES=ON "{cwd}"\
...
def remove_absl_utf8(dest_folder):
    os.chdir(dest_folder)
    shutil.rmtree('./include/absl')
    shutil.rmtree('./lib/cmake/absl')
    shutil.rmtree('./lib/cmake/utf8_range')
    for filename in glob('./include/utf8*.h'):
        os.remove(filename)
    for filename in glob('./lib/libabsl*'):
        os.remove(filename)
    for filename in glob('./lib/libutf8*'):
        os.remove(filename)
    for filename in glob('./lib/pkgconfig/absl*'):
        os.remove(filename)
    for filename in glob('./lib/pkgconfig/utf8*'):
        os.remove(filename)
...
def place_files(ndk_abi, dest_abi_code, src_folder: Path, aar_folder: Path):
    cmake_folder = aar_folder / "cmake"
    if not cmake_folder.exists():
        (src_folder / "lib"/"cmake").rename(cmake_folder.absolute())
...
def main(argv):
    ...
    if '-' in __package_name: __package_name = __package_name[:__package_name.index('-')]
